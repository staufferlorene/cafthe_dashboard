...
<head>
    ...
    <title>{if $action == 'add'}CafThé - Ajout client{else}CafThé - Modifier client{/if}</title>
...
<body>
    ...
    <h4 class="m-0 mb-3 font-weight-bold text-primary">{if $action == 'add'}Ajouter un client{else}Modifier un client{/if}</h4>

    {if $action == 'add'}
        <form action="index.php?action=add_client" method="post">
    {else}
        <form action="index.php?action=update_client&Id_client={$client.Id_client}" method="post">
    {/if}
